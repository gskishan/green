...
def get_conditions(filters):
    ...
    if filters.get("from_date") and filters.get("to_date"):
        condition += " AND date BETWEEN '{0}' AND '{1}' ".format(filters.get("from_date"), filters.get("to_date"))

    if filters.get("status"):
        condition += ' AND status="{0}" '.format(filters.get("status"))
    if filters.get("territory"):
        condition += ' AND territory="{0}" '.format(filters.get("territory"))
    return condition
